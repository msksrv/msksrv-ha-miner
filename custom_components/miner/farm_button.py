...
async def async_setup_farm_buttons(
    ...
    async_add_entities(
        [
            ...
            FarmClearEmergencyStopButton(coordinator),
...
class FarmClearEmergencyStopButton(CoordinatorEntity[MinerFarmCoordinator], ButtonEntity):
    """Clear farm emergency stop latch (two-step confirm within 30 s)."""

    _attr_has_entity_name = True
    _attr_translation_key = "farm_clear_emergency_stop"
    ...
    def __init__(self, coordinator: MinerFarmCoordinator) -> None:
        ...
        self._attr_unique_id = (
            f"farm-{coordinator.config_entry.entry_id}-clear-emergency-stop"
        )
    ...
    @property
    def available(self) -> bool:
        return bool(self.coordinator.data.get("emergency_stop_active"))

    async def async_press(self) -> None:
        if not self.coordinator.emergency_clear_armed:
            self.coordinator.arm_emergency_clear()
            _LOGGER.warning(
                "Emergency stop clear armed for 30 s — press again to confirm"
            )
            return
        await self.coordinator.async_clear_emergency_stop()
